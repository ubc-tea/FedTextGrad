...
class MultiFieldTokenParsedEvaluation(MultiFieldEvaluation):
    ...
    def parse_output(self, response: Variable) -> str:
        ...
        # # Regular expression to extract the value between the tags
        # match = re.search(r'<ACCURACY>\s*(\d+)\s*</ACCURACY>', response_text)
        # if match:
        #     # Extract the matched value (the number between the tags)
        #     response = match.group(1)
        #     return response

        try:
            parts = response_text.split(self.parse_tags[0])
            if len(parts) < 2:
                raise ValueError(f"Parsing failed: '{self.parse_tags[0]}' not found in response_text.")
            
            response_part = parts[1].split(self.parse_tags[1])
            if len(response_part) < 2:
                raise ValueError(f"Parsing failed: '{self.parse_tags[1]}' not found after splitting with '{self.parse_tags[0]}' in response_text.")

            response = response_part[0].strip()
        except IndexError as e:
            print(f"IndexError encountered: {e}")
            response = None  # or some default value
